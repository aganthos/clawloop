...
Clone-based usage (thin shim in examples/demo_math.py delegates here):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=_positive_int,
        default=None,
        help="Override number of learning iterations (must be >= 1)",
    )
    parser.add_argument(
        "--episodes",
        type=_positive_int,
        default=None,
        help="Override episodes per iteration (must be >= 1)",
    )
    parser.add_argument(
        "--output",
        type=str,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if iterations <= 0:
        ...
        sys.exit(1)
    if episodes <= 0:
        log.error("--episodes must be a positive integer, got %d", episodes)
        sys.exit(1)
