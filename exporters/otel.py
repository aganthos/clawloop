...
class OTelExporter(TraceExporter):
    """Export :class:`~clawloop.core.episode.Episode` objects as OTel/OpenInference spans.
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(global_provider, ProxyTracerProvider):
            ...
            self._tracer = global_provider.get_tracer("clawloop.exporters.otel")
            return
        ...
        self._tracer = provider.get_tracer("clawloop.exporters.otel")
    ...
    def _export_episode(self, ep: Episode) -> None:
        ...
        # Episode identity
        root_span.set_attribute("clawloop.episode.id", ep.id)
        root_span.set_attribute("clawloop.state.id", ep.state_id)
        root_span.set_attribute("clawloop.task.id", ep.task_id)
        if ep.session_id:
            root_span.set_attribute("clawloop.session.id", ep.session_id)
            root_span.set_attribute("gen_ai.conversation.id", ep.session_id)
        if ep.bench:
            root_span.set_attribute("clawloop.bench", ep.bench)

        # Rewards
        root_span.set_attribute("clawloop.reward.effective", summary.effective_reward())
        root_span.set_attribute("clawloop.reward.normalized", summary.normalized_reward())
        root_span.set_attribute(
            "clawloop.reward.signals",
            json.dumps(
                {
        ...
        root_span.set_attribute("clawloop.filtered", summary.filtered)

        # Per-signal attributes
        for name, sig in summary.signals.items():
            root_span.set_attribute(f"clawloop.reward.{name}.value", sig.value)
            root_span.set_attribute(f"clawloop.reward.{name}.confidence", sig.confidence)
        ...
        if harness_version is not None:
            root_span.set_attribute("clawloop.harness.version", str(harness_version))
        ...
        for msg_idx, msg in enumerate(ep.messages):
            ...
            # Step metadata
            llm_span.set_attribute("clawloop.step.index", step_idx)
            if step_idx < len(ep.steps):
                step_meta = ep.steps[step_idx]
                llm_span.set_attribute("clawloop.step.reward", step_meta.reward)
                llm_span.set_attribute("clawloop.step.done", step_meta.done)
            ...
            if msg.tool_calls:
                for tc in msg.tool_calls:
                    ...
                    tool_span.set_attribute("clawloop.step.index", step_idx)
