...
The robot SVG in `webpage/index.html` (in the "What is an AI Agent?" section, around line 214-271) looks off on the light background. It was adapted from the pitch deck (`business/pitch/deckv3.html`, lines 1170-1338) but the adaptation is rough.

## Reference: the good version

The pitch deck at `business/pitch/deckv3.html` has the correct robot+globe+arrows SVG in slide 7 (around lines 1170-1338). That version:
...
- **Edit**: `business/webpage/index.html` — the inline SVG between `<!-- Robot + Globe SVG -->` and the closing `</svg>` (around line 215-271)
- **Reference**: `business/pitch/deckv3.html` lines 1170-1338 — the good robot+globe SVG to adapt from
- **Also reference**: `business/resources/agent_environment_ppt.svg` — a simpler version of the same diagram
